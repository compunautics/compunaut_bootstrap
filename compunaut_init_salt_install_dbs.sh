...
# Install databases
  ...
  salt -C 'I@mysql:server:*' state.apply compunaut_mysql.repo,compunaut_mysql.mycnf,mysql.server,compunaut_influxdb,compunaut_mysql.mine --async

  echo_blue "Installing LDAP"
  salt -C 'I@openldap:slapd_services:*' state.apply compunaut_openvpn.deploy,compunaut_openldap --state_output=mixed -b1

  update_data
  echo_blue "Setting up Galera"
  salt -C 'I@mysql:server:*' state.apply compunaut_mysql.galera --state_output=mixed

  echo_blue "Complete MySQL setup"
  salt -C 'I@mysql:server:*' state.apply compunaut_mysql --state_output=mixed
